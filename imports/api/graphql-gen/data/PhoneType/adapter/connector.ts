...
let logger = log4js.getLogger('api:connector:PhoneType');

import { MongooseApi } from 'oda-api-graphql';
import PhoneTypeSchema from './schema';
import RegisterConnectors from '../../registerConnectors';
import * as Dataloader from 'dataloader';

import { PartialPhoneType } from '../types/model';
import { PhoneTypeConnector } from './interface';

export default class PhoneType extends MongooseApi<RegisterConnectors, PartialPhoneType> implements PhoneTypeConnector {
  constructor({mongoose, connectors, user, owner, acls, userGroup}) {
    logger.trace('constructor');
    super({mongoose, connectors, user, acls, userGroup, owner    });
    this.initSchema('PhoneType', PhoneTypeSchema());
  ...
  public async create(payload: PartialPhoneType) {
    logger.trace('create');
...
/*   public async _findOneById(id?: string) {
    ...
    if (!condition) {
      logger.error('no unique key provided findOneById "PhoneType"');
      throw new Error('no unique key provided findOneById "PhoneType"');
    }
...
/*   public async _findOneByName(name?: string) {
    ...
    if (!condition) {
      logger.error('no unique key provided findOneByName "PhoneType"');
      throw new Error('no unique key provided findOneByName "PhoneType"');
    }
    let result = await this.model.findOne(condition).exec();
    return result;
  } */

  public getPayload(args: PartialPhoneType, update?: boolean) {
    let entity: any = {};
