...
let logger = log4js.getLogger('api:connector:EmailType');
...
import { EmailTypeConnector } from './interface';

export default class EmailType extends MongooseApi<RegisterConnectors, PartialEmailType> implements EmailTypeConnector {
  constructor({mongoose, connectors, user, owner, acls, userGroup}) {
    logger.trace('constructor');
    super({mongoose, connectors, user, acls, userGroup, owner    });
    this.initSchema('EmailType', EmailTypeSchema());
  ...
  public async create(payload: PartialEmailType) {
    logger.trace('create');
...
/*   public async _findOneById(id?: string) {
    ...
    if (!condition) {
      logger.error('no unique key provided findOneById "EmailType"');
      throw new Error('no unique key provided findOneById "EmailType"');
    }
...
/*   public async _findOneByName(name?: string) {
    ...
    if (!condition) {
      logger.error('no unique key provided findOneByName "EmailType"');
      throw new Error('no unique key provided findOneByName "EmailType"');
    }
    let result = await this.model.findOne(condition).exec();
    return result;
  } */

  public getPayload(args: PartialEmailType, update?: boolean) {
    let entity: any = {};
