...
export const query: { [key: string]: any } = {
  emailTypes: async (
    ...
    let list = get(selectionSet, 'edges.node') ? await context.connectors.EmailType.getList({
      ...args,
      idMap,
    }) : [];
      ...
      let pageInfo = get(selectionSet, 'pageInfo') ?
        {
          ...
          count: get(selectionSet, 'pageInfo.count') ? await context.connectors.EmailType.getCount({
              ...args,
              idMap,
            }) : 0,
        } : null;
  ...
  emailType: async (
    ...
    logger.trace('emailType');
    let result;
    if (args.id) {
      result = await context.connectors.EmailType.findOneById(getValue(args.id));
    } else if (args.name) {
      result = await context.connectors.EmailType.findOneByName(args.name);
    }
    return result;
  },
};