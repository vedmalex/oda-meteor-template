...
export const query: { [key: string]: any } = {
  phones: async (
    ...
    let list = get(selectionSet, 'edges.node') ? await context.connectors.Phone.getList({
      ...args,
      idMap,
    }) : [];
  ...
  phone: async (
    ...
    let result;
    if (args.id) {
      result = await context.connectors.Phone.findOneById(getValue(args.id));
    } else if (args.phoneNumber) {
      result = await context.connectors.Phone.findOneByPhoneNumber(args.phoneNumber);
    }
    return result;
  },
};