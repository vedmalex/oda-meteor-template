...
export const query: { [key: string]: any } = {
  phoneTypes: async (
    ...
    let list = get(selectionSet, 'edges.node') ? await context.connectors.PhoneType.getList({
      ...args,
      idMap,
    }) : [];
      ...
      let pageInfo = get(selectionSet, 'pageInfo') ?
        {
          ...
          count: get(selectionSet, 'pageInfo.count') ? await context.connectors.PhoneType.getCount({
              ...args,
              idMap,
            }) : 0,
        } : null;
  ...
  phoneType: async (
    ...
    logger.trace('phoneType');
    let result;
    if (args.id) {
      result = await context.connectors.PhoneType.findOneById(getValue(args.id));
    } else if (args.name) {
      result = await context.connectors.PhoneType.findOneByName(args.name);
    }
    return result;
  },
};